...
class HeatTransferSimulationTab:
    ...
    def _add_parameter_controls(self, form: Any) -> None:
        ...
        for name, label, minimum, maximum, step in (
            ...
            ("mimo_coupling_cutoff_fraction", "coupling cutoff fraction", 0.0, 1.0, 0.01),
            ("mimo_decoupling_lambda", "regularization lambda", 0.0, 1.0e9, 0.000001),
    ...
    def run_simulation_sys_id_for_G_ctrl(self) -> None:
        ...
        if self.sys_id_uniform_baseline.isChecked():
            self.prepared.set_uniform_temperature(self._sys_id_uniform_baseline_temperature())
    ...
    def _save_sys_id_results(self) -> None:
        ...
        try:
            ...
            self.sys_id_status_label.setText(
                self.sys_id_status_label.text() + f"\nG_ctrl populated but graph save failed: {exc}"
            )
    ...
    def _sys_id_uniform_baseline_temperature(self) -> float:
        if (
            ...
            and self.sys_id_keep_cryocooler_active.isChecked()
            and any(node.has_cryocooler for node in self.model.nodes.values())
        ):
            return float(self.inputs["T_cooler_setpoint"].value())
        return float(self.inputs["T_env_K"].value())
    ...
    def _read_params(self) -> SimulationParameters:
        return SimulationParameters(
            ...
            mimo_decoupling_lambda=float(self.inputs["mimo_decoupling_lambda"].value()),
    ...
    def _mimo_controller_should_run(self) -> bool:
        if self.input_mode.currentText() != "heater_inputs" or self.model is None:
            return False
        return any(_node_uses_mimo_controller(node) for node in self.model.nodes.values())
    ...
    def _refresh_sensor_readouts(self) -> None:
        ...
        temperatures = self._temperature_values()
        heater_powers = self.prepared.heater_power_by_node() if self.prepared is not None else {}
        ...
        for row, node in enumerate(sorted(readout_nodes, key=lambda item: item.node_id)):
            ...
            desired_text = ""
            error_text = ""
            if self._mimo_controller_should_run() and _node_uses_mimo_controller(node):
