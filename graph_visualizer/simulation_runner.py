...
class SimulationRunner:
    ...
    def _simulate(self, prepared, params, C_diag, sensors, heaters, cryo_idx) -> None:
        ...
        while not self._should_stop():
            ...
            if time.time() - last_ckpt_wall >= self.cfg.checkpoint_interval_s:
                ...
                # Redraw from the series just flushed, so an unattended run is
                # inspectable while it runs instead of only after it ends. Out of
                # process -- see _spawn_plot_refresh.
    ...
    def _spawn_plot_refresh(self) -> None:
        """Redraw the plots in a SEPARATE process, and do not wait for it.

        matplotlib faulted natively inside savefig on a 3.2 h run -- a Windows
        access violation in the font cache, laying out a tick label. No try/except
        can catch that; it kills the interpreter outright. Drawing once at finalize
        risked it once. Drawing at every checkpoint risked it every ten minutes,
        and a run 22800 s into a 100000 s simulation died to it.

        The figures are fully regenerable from timeseries.npz, which is flushed
        immediately before this call, so the child needs nothing from us and its
        death costs a stale PNG rather than the run. One at a time: if the previous
        refresh is still going, skip this round instead of piling processes up
        behind a slow draw.
        """
        previous = getattr(self, "_plot_process", None)
        ...
        try:
            self._plot_process = subprocess.Popen(
                [
                    sys.executable,
                    "-c",
                    "import sys; from graph_visualizer.simulation_runner import regenerate_plots; "
        ...
        except Exception as exc:  # noqa: BLE001 - a missing plot must never end a run
            self._log_event("plot_refresh_failed", f"{type(exc).__name__}: {exc}")

    def _reap_plot_refresh(self, timeout_s: float = 30.0) -> None:
        """Let an in-flight refresh finish before finalize redraws over it."""
        process = getattr(self, "_plot_process", None)
        if process is None:
            return
        try:
            process.wait(timeout=timeout_s)
        except Exception:  # noqa: BLE001 - kill a hung child rather than block the report
            try:
                process.kill()
            except Exception:  # noqa: BLE001
                pass
    ...
    def _write_plots(self) -> list[str]:
        ...
        try:
            ...
            def _plot(keys, title, ylabel, fname):
                ...
                _legend(ax)
                fig.tight_layout()
                # Write-then-rename, like the timeseries: this now runs periodically
                # during the run, so a kill mid-save must not leave a truncated PNG
                # where a readable one used to be. savefig cannot infer the format
                # from a ".png.tmp" name, so state it.
                target = self.plots_dir / fname
                staged = target.with_name(target.name + ".tmp")
                fig.savefig(staged, dpi=110, bbox_inches="tight", format="png")
                plt.close(fig)
