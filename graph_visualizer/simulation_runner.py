    ...
    def _simulate(self, prepared, params, C_diag, sensors, heaters, cryo_idx) -> None:
        ...
        while not self._should_stop():
            ...
            if time.time() - last_ckpt_wall >= self.cfg.checkpoint_interval_s:
                ...
                # inspectable while it runs instead of only after it ends. A 12 h
                # run previously showed nothing until finalize -- and a run killed
                # before that (or aborted by a guard) left a directory whose plots
                # had to be regenerated by hand. Each figure overwrites its own
                # filename, so plots/ always holds the newest view.
                self._write_plots()
                last_ckpt_wall = time.time()
